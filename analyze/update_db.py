"""
Update a record of all events, given a file full
of users.
"""

import csv
import json
import requests
import sqlite3

# TODO (jaylamb20@gmail.com):
# implement support for Github auth so you don't have to worry
# about rate limiting

# set params
CSV_FILE = "thing.csv"
DB_FILE = "thing.db"
API_URL = "http://localhost:5090"

# Grab a cursor
conn = sqlite3.connect(DB_FILE)
cur = conn.cursor()

# Read in csv of users
with open(CSV_FILE) as f:
    x = csv.reader(f, delimiter=',')
    csv_rows = [r for r in x]

headers = csv_rows.pop(0)

user_insert_sql = "INSERT INTO users({}) VALUES({})".format(
    ",".join(headers),
    ",".join(['?'] * len(headers))
)

for user in csv_rows:
    try:
        cur.execute(user_insert_sql, user)
        conn.commit()
    except sqlite3.IntegrityError:
        print("User {} already exists".format(user[0]))

# Grab users from the database
# TODO (jaylamb20@gmail.com):
# add idea of updated_date and then pull last-updated
# (but still valid)
cur.execute('SELECT user_name FROM users')
user_names = [x[0] for x in cur.fetchall()]
conn.commit()

# Get events for each user, then write them to the DB
event_fields = ["id", "created_at", "type", "repo_name", "evidence_url"]
event_insert_sql = "INSERT INTO events({},user_name) VALUES({},?)".format(
    ",".join(event_fields),
    ",".join(['?'] * len(event_fields))
)

for user_name in user_names:
    print("Updating events for '{}'".format(user_name))
    res = requests.get("{}/api/events?user={}".format(API_URL, user_name))
    events = res.json()['events']
    duplicates = 0
    inserts = 0
    for event in events:
        try:
            cur.execute(
                event_insert_sql,
                [event[k] for k in event_fields] + [user_name]
            )
            conn.commit()
            inserts += 1
        except sqlite3.IntegrityError:
            duplicates += 1

    print("Done updating events for user '{}' ({} inserts, {} dupes)".format(user_name, inserts, duplicates))

print("Done updating events")
